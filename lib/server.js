...
var startServer = exports.startServer = function startServer(port) {
  ...
  var server = _net2.default.createServer(function (socket) {
    _winston2.default.info('Client connected');
    ...
    socket.on('data', function (data) {
      ...
      if (buffLen + dataLen > Config.MAX_DATA_BUFFER_SIZE) {
        // Reject
        _winston2.default.error('Buffer length exceded, resetting.');
      } else {
        ...
        if (chunks.length > 1) {
          ...
          _underscore2.default.each(payloads, function (payload) {
            _winston2.default.info('Received', payload);
  ...
  server.on('listening', function () {
    _winston2.default.info('Listening on port', port);
  ...
  server.on('error', function (err) {
    if (isFileSocket && err.code === 'EADDRINUSE') {
      _winston2.default.info('Address in use, deleting socket file.');
      _fs2.default.unlinkSync(port);
      _winston2.default.info('Start server again');
      server.listen(port);
    } else {
  ...
  // Start server.
  _winston2.default.info('Start server');
  server.listen(port);
};
